...
export class MouseDownLongItem
{
    ...
    onMouseDown(e:Event)
    {
        if(this.toucheId == -1)
        {
            this.mouseDownTimestamp = Engine.timer.currTimer;
            ...
            Engine.timer.once(this.longMillisecond, this, this.onDealyMouseDown);
...
export default class MouseDownLong 
{
   ...
   static getOrCreate(displayObj:any, listenCaller: any, listenFun:Function): MouseDownLongItem
   {
        ...
        if(!item)
        {
            item = Engine.Pool.createByClass(MouseDownLongItem);
